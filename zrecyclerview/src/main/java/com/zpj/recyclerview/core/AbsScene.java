...
public abstract class AbsScene<T extends Layouter> implements Scene {
...
//    }
    ...
    public AbsScene(MultiData<?> multiData, T layouter) {
    ...
    public void layoutChildren() {
        mLayouter.layoutChildren(this, mAnchorInfo);
    }
    ...
    public int scrollHorizontallyBy(int dx) {
        ...
        if (isOverScrolling) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                final Scene scene = mHelper.getScene(view);
                if (scene == this) {
                    for (int j = i; j < getChildCount(); j++) {
                        View child = getChildAt(j);
                        if (mHelper.getScene(child) != this) {
                            break;
                        }
...
//                    Log.d(TAG, "scrollHorizontallyBy dx=" + dx + " overScrollRadio=" + overScrollRadio + " overScroll=" + overScroll);
        ...
        if (dx > 0) {
            ...
            for (int i = getChildCount() - 1; i >= 0; i--) {
                View view = getChildAt(i);
                if (mHelper.getScene(view) == this) {
                    index = i;
                    consumed += fillHorizontal(view, dx);
                    break;
                }
            }
        } else {
        ...
        if (dx > 0) {
            ...
            for (int i = getChildCount() - 1; i >= 0; i--) {
                View view = getChildAt(i);
                Scene scene = mHelper.getScene(view);
                if (scene != this) {
                    continue;
                }
    ...
    public int fillVertical(View anchorView, int dy) {
        mAnchorInfo.anchorView = anchorView;
        if (anchorView == null) {
            if (dy > 0) {
                mAnchorInfo.y = getTop();
            } else {
                mAnchorInfo.y = getBottom();
            }
        } else {
            mAnchorInfo.position = getPosition(anchorView) - mPositionOffset;
            if (dy > 0) {
                mAnchorInfo.y = getDecoratedBottom(anchorView);
            } else {
                mAnchorInfo.y = getDecoratedTop(anchorView);
            }
        }
        int consumed = mLayouter.fillVertical(this, mAnchorInfo, dy);
        mAnchorInfo.anchorView = null;
        return consumed;
    }

    @Override
    public int fillHorizontal(View anchorView, int dx) {
        mAnchorInfo.anchorView = anchorView;
        int consumed = mLayouter.fillHorizontal(this, mAnchorInfo, dx);
        mAnchorInfo.anchorView = null;
        return consumed;
    }
}
